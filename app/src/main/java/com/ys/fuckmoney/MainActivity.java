...
public class MainActivity extends BaseActivity
        implements NavigationView.OnNavigationItemSelectedListener,View.OnClickListener, AccessibilityManager.AccessibilityStateChangeListener {
    ...
    private boolean checkAccPermission() {
        ...
        for (AccessibilityServiceInfo info : infos){
            if (info==null|| TextUtils.isEmpty(info.getId()))
                continue;
            if (info.getId().equals("com.ys.ts/.FMService")){
                return true;
            }
        }
        return false;
    ...
    public void onAccessibilityStateChanged(boolean enabled) {
        if (enabled){
            startService(new Intent(MainActivity.this,FuckMoneyAccService.class));
            mBtnStart.setText("服务已开启");
        }
    }
